---
title: "King Kong: Myth of Terror Island Rulesheet"
discourse_topic_id: 9514
manufacturer: "Stern Pinball"
opdb_id: "GEL0V"
source: https://tiltforums.com/t/king-kong-myth-of-terror-island-rulesheet/9514
---
...
- [Main Objectives](#heading--objectives)
  - [Island Scenes / Gong](#heading--island)
    - [Island Locks, Island Jackpot & Victory Challenge](#heading--lock)
...
- Original Wiki Rulesheet hosted on [Tilt Forums](/stern/king-kong-myth-of-terror-island-rulesheet_GEL0V)
...
- **Mega Secret Skill Shot**: Plunge to the upper left flipper and make a reflex shot to the cliffs ramp. Scores 1.75M and 8 seconds of ball save time. The skill shot is multiplied by 2x or 3x, with follow-up shots to the biplane ramp, or U-turn, respectively.
- **Ultra Secret Skill Shot**: Plunge to the upper left flipper, make a reflex shot to the cliffs ramp, then follow up with a shot from the upper right flipper, straight across the playfield, into the river lane. Scores 7M(?) and 8 seconds of ball save time.

On default settings, each skill shot adds more initial ball-save time.

Each *unique* skill shot made during a game instantly adds a letter to **[KING KONG](#heading--trex)**.

If the ball falls back into the shooter lane during single-ball play, a "Sneak-In" award starting at 1M is earned, and the player may attempt another Skill Shot. The "Pisang Raja" **[banana combo](#heading--banana)** requires this to happen off a ricochet from the Pit targets.
...
- **Save Ann**: Shoot lit shots to build the value at the gong shot. Lit shots score 250K + 25K per shot and increase the gong value by 500K + 50K per shot, with combo shots scoring and increasing the value by 2x. The gong value starts at 4M. At least one lit shot must be made to light the gong each time. Shoot the gong shot 3 times to light the final shot at the gong on a hurry-up timer to finish the mode. The final shot's value starts at the total value of the three lit gong shots that were collected prior. On the Premium / LE, each gong shot will allow the ball to enter the VUK during this mode.
- **Cross the Chasm**: The river entrance (Log Bridge insert, corresponding to the switch above the log diverter) is lit for the entire duration of the mode, hitting it automatically raises the diverter and lights the cliffs ramp for a few seconds. Shoot the cliffs ramp to save the sailors from falling off the bridge. Each explorer saved collects the base value starting at 1.5M and increments it by 500K. Fail to shoot the cliffs ramp in time and a sailor will be lost, resetting the base value to 1.5M. Comboing the U-turn immediately after saving a sailor awards the base value x a multiplier that increases with each lit U-turn shot. Rescue 6 sailors to complete.
- **Pterodactyl Attack**: Hit the lit moving drop target to defeat the pterodactyls for 5M and light the Kong Cave VUK on the far right of the playfield for 2x the award. A clean shot that triggers the rollover above the Kong Cave scoop makes the cave award 3x, and making the Kong Cave without the assistance of the upper flipper adds 3M to the base value. Collect three cave awards to complete, with the second cave award adding a **[KING KONG letter](#heading--trex)**.
- **Stegosaurus Encounter**: Shoot slow flashing shots to take the easy path, or shoot quickly flashing shots to take the hard path. Each shot collects the base value and progress the mode, while each hard path shot also increases a multiplier by +1x for the rest of the mode. The base value starts at 2M and increments by 250K per shot, regardless of path. Shoot 7 shots in total to finish the mode.
    The shot sequence is as follows:

    |Shot # | Easy Path | Hard Path |
    | --- | --- | --- |
    | 1 | Center spinner | (none) |
    | 2 | Center ramp | Cliffs ramp |
    | 3 | Gong | U-turn |
    | 4 | Left orbit | Kong Cave VUK |
    | 5 | Biplane ramp | Left orbit |
    | 6 | Center ramp | U-turn |
    | 7 | Punchback target | (none) |
...
These features become enabled if the player activates an island scene mode with island locks:
- **Island Jackpot**: After scoring any *progress* shot in a 2-ball multiball island scene, shoot the cliffs ramp to light island jackpot at the pit. The next time the player shoots the pit, the player will score the island jackpot worth 5M + the total of mode shots up to the point where island jackpot was lit. Knocking the held ball into the pit targets with the other ball, within the 5-second timer, scores an additional 2x island jackpot. The progress shots for each mode are:
  - **Save Ann** - gong hits
...
- 100 ft - 5M + **[map segment](#heading--map)**
- 200 ft - Light **[island lock](#heading--lock)** + light **[extra ball](#heading--extraballs)**
- 300 ft - 10M + light **[NYC event 1](#heading--nyc)**
- 400 ft - Spot **[KING KONG letter](#heading--trex)**
...
**NYC Event Modifiers:** After shooting the right orbit, use the left flipper to choose the modifier for the mode:

- *3-ball multiball (1x scoring)*
  - If less than 3 balls are in-play, advancing a stage will typically add a ball.
  - Draining back down to 1 ball ends the mode.
...
- *Limited flip count (2x scoring)*
  ...
  - Completing the mode awards bonus points for each flip left.
- *Single-ball, timed (3x scoring)*
  - Starts with a 12-second ball saver.
  - 60-second time limit, each phase completed adds time, but timer cannot exceed 60 seconds.
  - Completing the mode awards bonus points based on time left.

Each modifier can only be chosen once; further NYC Events will force you to choose among those that have not been chosen yet.

- **Stage Fright**: Kong has been captured and needs to escape. Shoot the lit feature to progress, then shoot the punchback target (via right orbit) to break a chain and complete the phase for 2.5M + 250K per phase + 500K per biplane ramp. When the punchback target is lit, features from previous phases are lit red for +1x chain break value. The biplane ramp is always lit to boost values. Completing the fifth right orbit shot ends the mode in success and awards a **map segment**.
  - Phase 1: 2 center ramp shots for 250K then 300K. Biplane ramp adds ???.
  - Phase 2: 80 right spinners for 5K + 250 per spin + ??? per biplane ramp.
  - Phase 3: Complete drop targets for 500K + ??? per biplane ramp.
  - Phase 4: 60 left spinners for 5K + 250 per spin + ??? per biplane ramp.
  - Phase 5: Hit cave 2 times for ??? + ??? per biplane ramp. A clean shot that triggers the rollover above the cave before falling into the cave counts as both shots needed.
- **Window Shopping**: Kong is wrecking buildings trying to search for Ann. Shoot the lit "climb" shots to light red arrows for jackpots - the number of "climb" shots increases for every subsequent jackpot. Shooting the biplane ramp will spot a "climb" shot or add +1x to the jackpot if it is already lit - the jackpot multiplier resets every time one is collected. The mode ends in success and a **map segment** is awarded once six jackpots have been scored.
  - Multiball perk: The first two jackpots will add balls and the third & fourth jackpots will give short ball save timers.
  - Single-ball timed perk: Each jackpot adds +10 seconds to the timer. Completing the drop targets at any time adds +25 seconds to the timer.
- **Manhattan Rampage**: Kong is causing havoc on the streets of Manhattan. Light each major ramp & orbit shot solid by completing them as many times as there are lights in front of them. Complete two shot lanes by lighting up all of their inserts to light a super jackpot (multiball) or light the kong cave (limited flips, single-ball) to complete the mode and award a **map segment**.
  - Multiball perk: Super jackpot is lit at the U-turn upper loop every time a shot is lit completely solid +1x for every shot that was lit prior. The first two super jackpots add a ball. Every super jackpot requires more shot lanes to be completed, to a maximum of 5; and the U-turn will spot shots if super jackpot isn't lit.
...
There are two combos that benefit Biplane Attack and can be scored at any time during single-ball play:
- **Barrel roll**: left orbit - side ramp. Adds +250k to the next biplane destroyed.
- **Chandelle maneuver**: river - punchback target - side ramp. Adds +500k to the next biplane destroyed and +15 seconds to the mode's timer.
...
- Making a **[skill shot](#heading--skillshots)** (one letter earned per unique skill shot)
- Collecting enough switch hits to fill the green bar on the left of the display (lit at the gong, progress is doubled at maximum **[powerup](#heading--kombos)** level)
- Scoring 3, 15, etc. **[cliffs ramp shots](#heading--cliffs)** (lit at the gong)
- Making 2 lit Kong Cave shots during the **pterodactyl attack** **[island scene](#heading--island)** (awarded instantly)
- Collecting a flawless **[super sweep](#heading--kombos)** by sweeping all four drop targets before shooting the side ramp (awarded instantly)

Each KING KONG letter adds +1 **[log bridge](#heading--bridge)** use at the action button and awards a different perk that applies to the next shot of a certain type or lights the cliffs ramp for **light island jackpot**.

**Set 1**

- **K**: 5M
- **I**: Boost **[king kombos](#heading--kombos)** +50k
- **N**: Next **[cliffs](#heading--cliffs)** shot awards 2x progress
- **G**: Light **[deadeye](#heading--deadeye)** (Pro); boost **king kombos** +50k (Prem / LE)
- **K**: Next **[river](#heading--river)** shot awards 2x progress
- **O**: Boost **king kombos** + 50k (Pro); light **deadeye** (Prem / LE)
- **N**: Next **[pit](#heading--spider)** shot awards 2x progress + **[map segment](#heading--map)**
- **G**: Light **T-Rex Battle**

**Set 2**

- **K**: 5M
- **I**: Boost **king kombos** + 50k
- **N**: Next round of **[biplane attack](#heading--plane)** worth 2x
- **G**: Light **deadeye** (Pro); boost **king kombos** +50k (Prem / LE)
- **K**: Start **super kombos**
- **O**: Boost **king kombos** + 50k (Pro); light **deadeye** (Prem / LE)
- **N**: ?
- **G**: Light **T-Rex Boss Battle**
...
The deadeye award starts at 1M and increases by 500k for each award collected. Certain skill shots multiply the value: the pit standups for 1x; left outlane is 2x; left inlane is 3x; outer-left inlane is 5x. **[2x playfield](#heading--2x)** can also be brought into Deadeye (with a unique callout for hitting the Deadeye jackpot at 2x), but times out like it normally does.
...
Though all green shots during King Kong multiball score smaller awards with a Kong award for 5x at the side ramp, the jackpots (red arrows) must be lit. For the first jackpot round of a game, hitting any drop target will light the center spinner and punchback target for 1x jackpot. After this first jackpot round, all drop targets must be completed to light jackpots and the center spinner won't award jackpots; instead, the cliffs ramp will be lit to score a 2x jackpot.

After scoring either jackpot, shoot the side ramp to score a super jackpot. The jackpot increases as green arrow shots are made and is multiplied as side ramps are made prior to collecting the super jackpot. The super jackpot starts the game lit for about 30 seconds, but every subsequent jackpot scored reduces the timer that the super jackpot is available for.

A ball can be relocked in the train by collecting all of the green arrow awards, then shooting the center ramp. The relock multiplies all jackpots & super jackpots by 2x and lasts 20 seconds before releasing the ball. On the Pro, the ball is not physically held during the timed 2x jackpots.
...
After playing all three pit multiballs, the fourth pit multiball will be **Spider Queen** 3-ball multiball. The spider queen's spawn must be defeated by hitting randomly lit major shots and then scoring the super jackpot at the pit to deal damage to the queen. The super jackpot starts at 5M + 50% of the total from the 3 previous pit multiballs. Every super jackpot scored means more jackpots to make before the next one is lit: 4 for the first 2 super jackpots, 5 for the 3rd, 6 for the 4th, and 7 for all subsequent supers. The jackpot value starts at 5M and increases by 1M per super jackpot made.

The mystery add-a-ball is disabled during Spider Queen multiball. Instead, if the player is below 4 balls during the multiball, hitting the island treasure standup behind the drop targets will add a ball; up to 2 balls can be added this way.
...
Awards are given for every treasure discovered in a game. Alongside each treasure giving a certain super mode or perk, perks are also given after collecting treasure from either method combined during a game:

- 1 treasure: adds +8 seconds to all **[island scene](#heading--island)** timers.
- 2 treasures: adds +3 seconds to the period that the **[powerup target](#heading--kombos)** is lit for.
- 3 treasures: all subsequent **[treasure hunt](#heading--map)** jackpots this game require one less shot to light.
- 4 treasures: adds +15 seconds to the ball saver during **[T-Rex battle](#heading--trexbattle)**.
- 5 treasures: all subsequent **[Lost Temple](#heading--glyph)** shots this game will spot 2 ciphers by default.
- 6 treasures: easier **[KONG letters](#heading--trex)** - halves the number of switch hits needed to light them.
- 7 treasures: lights **[cliffhanger](#heading--cliffhanger)** and increases the time limit for it by +5 seconds.
- 8 treasures: all subsequent **treasure hunt** jackpots this game require one less shot to light.
- 9 treasures: all **[biplane attack](#heading--plane)** awards now score 5x their normal value.
...
- **Drillplate of Dunne**: 10M + light **Super Spinners** (starts on the next spinner rip: alternate between left and right spinners for 100k per spin + 50k per lit spinner rip for 30 seconds. Spinner rips cap out at 25M, at which point the opposite spinner must be hit)
- **Lost Skull of the Ancients**: 10M + light **Super Kombos** (starts on the next shot made: starts **[King Kombos](#heading--kombos)** for 30 seconds but at 3x value per shot)
- **Heart of the Keepers**: 10M + light **Lost Temple Multiball** (starts at the gong once no other modes are running. 3-ball multiball where one drop target is lit at a time to increase the jackpot multiplier up to 4x. Super jackpot is at the island treasure target & resets the drops)
- **Moonstone of N'zai**: 10M + **[extra ball](#heading--extraballs)**
...
- **Island scenes** level 2 (completed all 5 island scenes)
- **[Climb](#heading--climb)** 100ft & 700ft
- Complete any **[NYC event](#heading--nyc)**
- **Climb** level 2 (2 buildings fully climbed)
- **[Biplane attack](#heading--plane)** level 2 (scored a 25M biplane attack award)
- Collecting the second "N" in **[KING KONG](#heading--trex)**
- Scoring the deadeye jackpot during **[deadeye](#heading--deadeye)**
- **[T-Rex Battle](#heading--trex)** level 2 (defeated both T-Rex)
- **[Kong Multiball](#heading--kong)** level 2 (collected 2 super jackpots)
- **[Spider Pit](#heading--spider)** level 2 (scored 250M in spider queen multiball)
- Collecting the rapids jackpot during **[rapids](#heading--river)**
- 20 **[cliffs](#heading--cliffs)** ramp shots
- Collecting 8 **[banana combos](#heading--banana)**
- Rare **[mystery award](#heading--mystery)** (once per game)

Once the map is completed, the Kong cave will light to start a **treasure hunt**. Make six randomly lit gold shots to follow the directions on the map - the final shot will always be at the kong cave - and collect a perk that changes with every treasure hunt complete. The shots required increase by 1 with each treasure hunt and reduce by 1 after collecting 3, then 8 **[treasures](#heading--treasures)** in a game.

Making the final shot of a treasure hunt awards the treasure hunt jackpot, which starts at 20M and increases as switches are hit during the hunt. The treasure hunt jackpot is multiplied by +1x for every **treasure** (either from treasure hunts or lost temple) that has been found during the game.

If the ball drains during a treasure hunt, then the treasure will be lost. Treasure hunts can run at any time during the game, even during other modes, but can only start during single-ball play.

The awards for completing each treasure hunt are:
- **Map 1** (Crown of the Forgotten God) - Add +15 seconds to all super mode timers. This applies to the four super modes from **[island treasures](#heading--glyph)** 1, 2, 4, and 6.
- **Map 2** - (Lost Idol of the Ancients) - Double **[end-of-ball bonus](#heading--bonus)** for the rest of the game.
- **Map 3** (Echo Horn) - The player now starts every ball with maximum **[log bridge](#heading--bridge)** uses.
- **Map 4** (Jade Goat of Sternnes) - Easier to start **[2x scoring](#heading--2x)**. All targets will now be lit to start it or increase its timer, instead of only one being lit at a time.
- **Map 5** (Sphere of Dympleton) - 500M points. Cannot be multiplied by 2x scoring.

With the topper installed, every map completed will also spot a clue towards finding the Golden Gary. Complete 5 maps and shoot the Kong Cave to start the topper-exclusive mode.
...
- **Super Sweep**: A bonus is awarded if the player hits at least 2 drop targets in the same shot that passes through the left orbit, then shoots the side ramp to complete the combo for 10M. If all 4 drop targets are swept in the same shot, the side ramp will award 50M and spot a **[KING KONG letter](#heading--trex)**!
- **Ricochet**: Rolling through the right return lane, and then shooting the rubber near the mini-flipper (from the right flipper) and bouncing up the cliffs ramp scores 5M. The combo can only be earned in single-ball play.

King Kombos also light the **powerup target** to the right of the KONG drop targets. Every time a 2+ way kombo is made, the powerup target will be lit for as long as the kombo is currently active (alongside a short grace period). Hitting the powerup target while lit awards a powerup bonus worth the total of all kombos made x the length of the kombo leading up to the target hit), and starts a timed powerup award. Each shot added to the kombo adds one award to the powerup "pool", maxing out at 4 awards on a 5-way kombo. The maximum powerup bonus is 100M.

- 2-way kombo: All **[climb](#heading--climb)** arrows are flashing and will make building progress for the next 13 seconds.
- 3-way kombo: Above award + all **[pit target](#heading--spider)** hits score 2x progress for the next 16 seconds.
- 4-way kombo: Above awards + right spinner is always lit for **[lost temple](#heading--glyph)**. For the next 20 seconds, hitting the lit cipher will award 2x advances to lighting the next island treasure.
- 5-way kombo: Above awards + all switch hits award 2x progress towards lighting **[KING KONG letters](#heading--trex)** at the gong for the next 25 seconds.
...
- 5, 15, 25... : **Rapids**
- 8: **[Light Extra Ball](#heading--extraballs)**
...
|Manzano|Punchback target|Side ramp|Pit targets
...
- 10M points
- Light **[island scene](#heading--island)**
- Spot **[climb arrow](#heading--climb)**
- Spot **[biplane ramp shot](#heading--plane)**
- Spot **[KING KONG letter](#heading--trex)**
- Light **[King Kong lock](#heading--kong)**
- Advance **[pit](#heading--spider)**
- Collect **[map segment](#heading--map)** (once per game)
- Spot **[river shot](#heading--river)**
- +2 **[log bridge uses](#heading--bridge)**
- 173,000 + spot **[cliffs ramp shot](#heading--cliffs)**
- Spot **[lost temple glyph](#heading--glyph)**
- +3 bonus X
- Add-a-ball (always awarded when any **[island scene](#heading--island)** boosted by an **[island lock](#heading--lock)** and / or **[spider pit multiball](#heading--spider)** is active)
...
- After 25 **[cliffs ramp shots](#heading--cliffs)**
- After 8 lit **[river shots](#heading--river)**

and awarded instantly as the 4th **[island treasure](#heading--glyph)** in a game.
...
* 2,500 x **[height climbed](#heading--climb)**
* 150,000 x **[KING KONG letters](#heading--trex)** collected

All multiplied by the bonus X, if applicable, built at the drop targets. Each completion of the drop targets awards +1 bonus X. **[Island Mystery](#heading--mystery)** can also award +3 bonus X.

Height climbed and KING KONG letters collected are cumulative across balls, while switch hits and bonus X resets with each ball.
...
Once all 5 hurry-ups are scored, the biplane ramp will light to score the super jackpot, *or* the player can continue to complete sets of 5 hurry-ups to exponentially increase the super jackpot multiplier (2x, 4x, 8x, etc.). The base super jackpot value is worth the total of all hurry-up scores in the mode so far and resets along with its multiplier once it is scored.
...
If the player drains down to a single-ball during the mode, they will have 15 seconds to keep the mode going. The timer resets if Pummel is scored, or if a ball is added after defeating the first T-Rex. If time runs out or the ball drains, the mode ends.
