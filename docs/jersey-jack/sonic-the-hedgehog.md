...
- [Other Scoring](#heading--otherscoring)
  - [Ring Combos](#heading--rings)
  - [Advance Chao](#heading--chao)
  - [Badnik Attack](#heading--badnik)
  - [Spin Dash](#heading--spindash)
  - [Find Froggy](#heading--froggy)
  - [Tails Hurry-Up](#heading--tails)
  - [Chaos Control](#heading--control)
...
|5|Center ramp (2 shots)|Chao target|Left lab orbit (2 shots)|
|Goal Ring|N/A|Center ramp|Lab jump ramp|
...
|Checkpoint #|**1x**|**2x**|**3x**|
| --- | --- | --- | --- |
|Start|Left ramp|Right orbit|Right lab target|
|1|Right ramp|Left ramp|Right orbit|
|2|Lockup|Bash dash target|Lab loopback ramp|
|3|Center ramp|Froggy target|Lab spinner|
|4|Battle zone targets (6 hits)|Chao target|N/A|
|5|N/A|N/A|Left lab orbit (2 shots)|
|Goal Ring|N/A|Center ramp|Lab jump ramp|
...
|Checkpoint #|**1x**|**2x**|**3x**|
| --- | --- | --- | --- |
|Start|Center ramp|Left or right orbit|Left or right lab orbit|
|1|Left or right ramp (2 shots)|N/A|Lab loopback ramp|
|2|N/A|Bash dash target (2 shots)|Either lab target|
|3|Battle zone targets (6 hits)|Right orbit|Either lab ramp|
|4|Right ramp (2 shots)|Right orbit|Left lab orbit|
|5|Lockup (2 shots)|Bash dash target (2 shots)|Lab spinner|
|Goal Ring|N/A|Center ramp|Lab jump ramp|
...
Shoot the inline drop targets near the right ramp to lock balls and eventually start Chaos Emerald Multiball. The first multiball requires only one lockup shot to start, but subsequent ones require one more to a maximum of three locked balls.

While in multiball, along with scoring jackpots, the player can collect Chaos Emeralds lit in the following order: 1, 2 & 3, 4 & 5, 6, 7.

- **1**: Shoot lockup to collect.
- **2**: Shoot right ramp 2x to light, then lockup to collect.
- **3**: Shoot lab jump ramp to collect.
- **4**: Shoot lab loopback ramp to collect.
- **5**: Make 50 spins, then lockup to collect.
...
There are four teams of three heroes that can be earned by completing the HERO inlanes, which cycle with the flippers. Pressing the action button will change the currently active team.

As soon as the player has collected all three members of one team, the left standup target will start Sonic Heroes Multiball. The player can delay this multiball and collect more teams by pressing the action button during the intro animation. When a team is completed, their respective shot will score 1.25x its normal value (Chaotix - left orbit, Dark - left ramp, Sonic - right ramp, Rose - right orbit).

Shoot jackpots to increase the value per jackpot by 100k and light the super jackpot at the left standup target for each team after clearing a set of unique jackpots. Team Sonic shots are the battle zone targets & center ramp, Team Dark's are on the lab upper playfield, Team Rose's are near the right orbit, and Team Chaotix's are near the left orbit. If multiple super jackpots are lit (only obtainable by completing more than one team before starting the multiball), the super jackpot value will be multiplied. **Bash Dash** will also multiply the super jackpots, leading to massive scoring potential from this multiball. Add-a-ball lights at the lockup once the initial ball save timer expires.
...
Shadow, an artificial counterpart to Sonic created by Eggman's grandfather, has the power to slow down time with the power of the Chaos Emeralds.

If the player has any **[Chaos Emeralds](#heading--emerald)** in their possession, pressing the action button will freeze all currently active game timers for 15 seconds per emerald found, and pressing the action button again will reactivate the timers. Every emerald earned adds more time to Chaos Control.

Timers affected by Chaos Control are the timer for each **[zone](#heading--zones)** (making it easier to get a higher rank), the hurry-up award during **boss battle** and **[Tails hurry-up](#heading--tails)**, and the **[bash dash](#heading--multiballs)** timer during multiball modes.
...
If extra balls are disabled, 10M is scored as a compensation award.
